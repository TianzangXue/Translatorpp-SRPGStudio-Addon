...
const BasicEventHandler = require("www/js/BasicEventHandler");
const UIParts = require("www/js/ui.parts");
const NWUtil = require("www/js/NWUtil");
...
const legacyUILogger = require("www/js/ui.legacylog.js");
legacyUILogger(ui);
 ...
 * @param {jQuery} html - jQuery object to be rendered
 */
ui.renderExternalLinks = function(html) {
	html.find("a.externalLink, a[external]").off("click.openExternal");
	html.find("a.externalLink, a[external]").on("click.openExternal", function(e) {
...
ui.createMenu = function(menuItems) {
	if (typeof menuItems == 'undefined') return false;
	return common.nwUtil.createMenu(menuItems);
}

ui.popupMenu = function(menu, $wrapper) {
...
// apply logger functions
require("www/js/ui.log.js")(ui);
...
ui.confirmTranslatorRequiredOptionIsUnset = function(translatorEngine, requiredOptions=[], options={}) {
	const renderRequiredOptions = (requiredOptions) => {
		const $requiredOptions = $("<ul></ul>");
		for (let i=0; i<requiredOptions.length; i++) {
			(()=> {
				const thisOptions = requiredOptions[i];
				let $template = $(`<li><a href="#">${thisOptions.title}</a></li>`);
				$template.find("a").on("click", function(e) {
					ui.openOptionsWindow({focus: {menu: translatorEngine.id ,find: thisOptions.title}});
				});
				$requiredOptions.append($template);
			})();
	...
	return new Promise((resolve) => {
		...
		const $dialog = $(`<div>
			<h1 class='icon-attention warningTitle'>${t("One or more required options are not set for ")}${translatorEngine.name}</h1>
			<p class='warningContent'>
				${t("One or more settings required by the translator engine are not set. If you continue, the translation process may fail.")}
			</p>
			<h2>${t("Required options :")}</h2>
			<div class="requiredOptions">
			</div>
			</div>`);
		$dialog.find(".requiredOptions").append(renderRequiredOptions(requiredOptions));
		var result = false;

		$dialog.dialog({
			autoOpen: false,
			title:"Confirm: Unfilled required options",
			modal:true,
			//width:Math.round($(window).width()/100*80),
			//height:Math.round($(window).height()/100*80),
			width:640,
			...
			buttons:[
				{
					text: t("Open the settings"),
					icon: "ui-icon-close",
					click: function() {
						// do nothing
						ui.openOptionsWindow({focus: {menu: translatorEngine.id}})
					}
				},
				{
					text: t("Cancel"),
...
ui.translateAllDialog = function() {
	const $dialogTranslateAll = $("#dialogTranslateAll");	
	...
	// translator's specific options
	var customFields = null;
	const renderTranslatorOptions = async (translatorName) => {
		customFields = null;
		$dialogTranslateAll.find(".batchTranslationOptions").addClass("hidden");
		$dialogTranslateAll.find(".batchTranslationOptions .customField").empty();
		
		const engine = trans.getTranslatorEngine(translatorName);
		if (typeof engine?.batchTranslationOptions !== 'function') return;
		console.log("rendering translator's specific options", translatorName);
		$dialogTranslateAll.find(".batchTranslationOptions").removeClass("hidden");
		$dialogTranslateAll.find(".batchTranslationOptions .customField").append(`<div class="loading"><i class="icon-spin5 rotating-slow inline-block"></i> Loading...</div>`);
		const batchOption = await engine.batchTranslationOptions();
		customFields = new CustomFields(batchOption);
		$dialogTranslateAll.find(".batchTranslationOptions .customField").empty();
		$dialogTranslateAll.find(".batchTranslationOptions .customField").append(customFields.$elm);
	}
	const getTranslatorOptionsValue = () => {
		console.log("Batch option", customFields);
		if (!customFields) return {};
		return customFields.getFormValues();
	}
	
	const $translatorSelector = $dialogTranslateAll.find(".translatorSelector")
	$translatorSelector.empty();
	for (let i=0; i<trans.translator.length; i++) {
		$translatorSelector.append("<option value='"+trans.translator[i]+"'>"+trans[trans.translator[i]].name+"</option>");
	}
	$translatorSelector.off("change");
	$translatorSelector.on("change", function(e) {
		renderTranslatorOptions($(this).val());
	});
	$translatorSelector.val(trans.getActiveTranslator());
	renderTranslatorOptions(trans.getActiveTranslator());
	...
	engines.handler('onOpenTranslateAllDialogReady').call(this, $dialogTranslateAll, {});
	
	
	$dialogTranslateAll.dialog({
		autoOpen: false,
		modal:true,
		width:Math.round($(window).width()/100*75),
		height:Math.round($(window).height()/100*86),
		// width:800,
		// height:640,
		minWidth:480,
		minHeight:320,
		close: function() {
			console.log("ui translateAllDialog closed");
			$(this).dialog("destroy");
		},
		show: {
			effect: "fade",
		...
		buttons:[
			...
			},			
			{
				...
				click: function() {
					...
					const sourceColumn = parseInt($("#dialogTranslateAll .sourceCol select").val());
					options = options || {};
					...
					options.translatorOptions 	= getTranslatorOptionsValue();
					options.targetColumn		= parseInt($("#dialogTranslateAll .targetCol select").val());
					...
					console.log("Batch translate with following options:", options);
				...
				},			
				{
					...
					click: function() {
						...
						if (action == "appendTags") {
							...
							trans.removeTagForSelectedRow(tagList, cellSelection)
						}							
...
ui.openTranslatorPane = function(param, options) {
	...
	localStorage.setItem("transAggregatorParam", JSON.stringify({
		param: param,
		posTop: posTop,
		posLeft: posLeft,
	}));

	nw.Window.open("www/translator.html#"+param, {
		width:targetWidth,
		height:targetHeight,
		show: false,
	}, (win)=> {
		ui.windows['translator'] = win;
	});
...
ui.resize.editorHeight = function(size) {
	console.log("%cui.resize.editorHeight", "color:blue; font-weight:bold;", size);
	size = size || window.innerHeight - 290;
	sys.config = sys.config || {}
	sys.config.paneSizes = sys.config.paneSizes||{};
	...
	//if (typeof size == 'number') size = size+"px";
	// ensure that the height is not more than window height - 120px
	if (sizeInt > window.innerHeight - 120) sizeInt = window.innerHeight - 120 + "px";

	$(".panel-wrapper").css("height", sizeInt);
	const $cellInfoPane = $("#cellSelectionInfo");
	$cellInfoPane.css("height", (window.innerHeight - sizeInt - 90)+"px");
	$cellInfoPane.css("--panel-height", (window.innerHeight - sizeInt - 90)+"px");
	$cellInfoPane.css("top", "unset");
	trans.refreshGrid();	
	ui.fixCellInfoSize();			
}
...
ui.resize.init = function() {
	...
	$("#cellSelectionInfo").resizable({
		...
		resize:function(e, thisUi) {
			// console.log("e", e.offsetY, e.clientY);
			thisUi.element.css("top", "unset");
			$(".panel-wrapper").css("height", e.clientY - 64);
			$(".panel-wrapper").css("max-height", `${e.clientY - 64}px`);
			thisUi.element.css("--panel-height", thisUi.element.css("height"));
		...
		stop:function(e, thisUi) {
			...
			sys.config = sys.config || {}
			sys.config.paneSizes = sys.config.paneSizes||{};
			sys.config.paneSizes.editorHeight = $(".panel-wrapper").css("height");
			thisUi.element.css("top", "unset");
			thisUi.element.css("--panel-height", thisUi.element.css("height"));
			console.log("resized", e);
			// $(".panel-wrapper").css("height", e.clientY - 64);
			// $(".panel-wrapper").css("max-height", e.clientY - 64);
...
ui.openFileNote = function(param, options) {
	...
	const posTop = Math.ceil($(".menu-button.addNote").offset().top + window.screenY + 32);
	const posLeft = Math.ceil($(".menu-button.addNote").offset().left + window.screenX - 60);

	const {showModalDialog} = require("www/js/modalDialog.js");
	showModalDialog("www/note.html#"+param, options, {
		id:"note", 
		width:640, 
		height:320,
		dialogleft:posLeft,
...
ui.openSearchWindow = async function(param) {
	param = param||"";
...
ui.bringSearchMenuToMainScreen = async function() {
...
ui.openSearchMenu = function() {
	...
	ui.menus.search.append(new nw.MenuItem({
		...
		type: "normal", 
		click: async function(){
			ui.bringSearchMenuToMainScreen();
		}
	}));
...
ui.fixCellInfoSize = function() {
	...
	let cellInfoHeight = parseInt($("#cellSelectionInfo").css("--panel-height"));
	const mainAreaHeight = $("#mainArea").eq(0).outerHeight();

	if (!cellInfoHeight) {
		// set default height of one third of main area
		cellInfoHeight = Math.round(mainAreaHeight/3);
		$("#cellSelectionInfo").css("--panel-height", cellInfoHeight+"px");
	}
	//const mainPanelHeight = $(".panel-wrapper").eq(0).outerHeight();
	const panelWrapperHeight = Math.round(mainAreaHeight - cellInfoHeight);
	console.log("setting panelWrapperHeight height", panelWrapperHeight);

	$(".panel-wrapper").css("height", panelWrapperHeight);
	$(".panel-wrapper").css("max-height", panelWrapperHeight);
}
...
ui.openProjectProperties = async function(options) {
	options = options||{};
	if (!trans.project?.options) return;
...
/**
 * Open Options Window
 * Can handle both nwjs v2 and legacy
 * @param {Object} options - Options
 * @param {String} options.focus - Focus on specific tab
 * @param {String} options.focus.menu - Menu to open
 * @param {String} options.focus.find - Find text in the opened menu
 * @example
 ...
 */
ui.openOptionsWindow = async function(options={}) {
	options ||= {};
	options.focus ||= undefined;

	if (options.focus) {
		console.log("focus", options.focus);
		localStorage.setItem("optionWindowFocus", JSON.stringify(options.focus));
	}
	
	// opening options window while the window is opened will crash Translator++.
	if (!NWUtil.isNW2()) {
		// legacy code
		if (ui.windows['options']) {
			ui.windows['options'].focus();
			return;
		}

		ui.windows['options'] = window.open("/www/options.html", "options", "");
		return;
	}
	
	// nwjs v2
	const optionsNWWindow = await NWUtil.getAllNwWindowsWithTitle("Options - Translator++");
	console.log("optionsNWWindow", optionsNWWindow);
	if (optionsNWWindow.length > 0) {
		optionsNWWindow[0].focus();
		return;
	}

	// open new window
	nw.Window.open('www/options.html', {}, function(thisWin) {
...
ui.showLanguageSelector = async function(sl, tl, options={}) {
  // Set default options
  options = {
    left: 'center',
    right: 'auto',
    width: 600,
    height: 120, // Changed to auto for better fit with horizontal layout
    ...options
  };

  // Create a promise to handle the async resolution
  return new Promise((resolve) => {
      ...
      <div class="bs5 p-3">
        <div class="d-flex align-items-center justify-content-between gap-3 mt-3">
          <div class="flex-grow-1">
            <label class="form-label source-language-label visually-hidden">Source Language</label>
            <select class="form-select source-language-select"></select>
          </div>
          
          <div>
            <button type="button" class="btn btn-outline-secondary swap-languages-btn p-2" title="Swap languages">
          ...
          <div class="flex-grow-1">
            <label class="form-label target-language-label visually-hidden">Target Language</label>
            <select class="form-select target-language-select"></select>
          </div>
        </div>
      </div>
    `);
    ...
    const populateLanguageOptions = ($select, selectedValue) => {
      $select.empty();
      for (const [code, langData] of Object.entries(langTools.langDB)) {
		let name = langData.name;
		if (langData.displayName) name = `${langData.name} (${langData.displayName})`;
        $select.append(
          $('<option>', {
            value: code,
            text: name,
    ...
    populateLanguageOptions($content.find('.source-language-select'), sl);
    populateLanguageOptions($content.find('.target-language-select'), tl);

    // Create the dialog
    const $dialog = $('<div></div>').append($content).dialog({
      autoOpen: true,
      modal: true,
      width: options.width,
      ...
      position: {
        my: 'left top',
        at: 'left bottom',
        of: options.parentElement||window,
        collision: 'fit'
      },
      classes: {
        "ui-dialog": "bs5",
        "ui-dialog-titlebar": "none",
        "ui-dialog-buttonpane": "none"
      },
      closeOnEscape: true,
      resizable: false,
      draggable: false,
      close: function() {
        const result = {
          sl: $content.find('.source-language-select').val(),
          tl: $content.find('.target-language-select').val()
        };
    ...
    $content.find('.swap-languages-btn').on('click', function() {
      const $source = $content.find('.source-language-select');
      const $target = $content.find('.target-language-select');
      const temp = $source.val();
      $source.val($target.val());
      $target.val(temp);
    });

    // Close dialog when clicking outside
    $('.ui-widget-overlay').on('click.languageSelector', function() {
		$('.ui-widget-overlay').off('click.languageSelector');
		$dialog.dialog('close');
    });
  });
}
...
ui.openFileDialog = async function(options) {
	return new Promise(async (resolve, reject) => {
		...
		var value = undefined;
		// append top layer to the body
		const $topLayer = $("<div class='topLayer' style='width:100vw; height:100vh; position:fixed; top:0px; left:0px; background:rgba(0,0,0,0.2); z-index:9999999999'></div>");
		
		const $elm = $("<input type='file' style='display:none' class='hidden __pseudoFileDlg1' data-value='' />");
		// add mutation observer to $elm to track property changes
		...
		// 	attributes: true,
		// 	attributeFilter: ['data-value', 'data-closed'],
		// });
		...
		$elm.on("input", async function() {
			...
			console.log("input")
			resolve(value);
			await options.onSelect.call(this, value);
			ui.hideBusyOverlay();
			$(this).remove();
			$topLayer.remove();
		});
		...
		// if topLayer receive click, then immidiately resolve as empty
		$topLayer.one("click", function() {
		...
		// it is observed that sometime focus is triggered before the input popup show up
		...
		$(window).one("focus", async function() {
			...
			// if element value is set, then let the input event handle the rest
			if ($elm.val()) return;
			$elm.remove();
			$topLayer.remove();
			resolve("");
			//$elm.attr("data-closed", "true");
		});
	})
...
ui.Toolbar.prototype.addButton = function(buttonObj) {
	...
	if (buttonObj.html) {
		buttonObj.elm	= $(buttonObj.html);
	} else {
		buttonObj.elm	= buttonObj.elm || `<button class="menu-button" data-tranattr="title" title="${buttonObj.title}">${buttonObj.text}</button>`
		buttonObj.elm 	= $(buttonObj.elm);
	}
	if (buttonObj.className) buttonObj.elm.addClass(buttonObj.className);
	buttonObj.checked = buttonObj.checked || false;
	...
	} else if (buttonObj.html || buttonObj.text) {
		// do nothing
	} else {
...
/**
 * Add a button with expandable menu button next to it
 * @param {Object} buttonObj - Button object
 * @param {Object} menu - Menu object
 * @param {string} [buttonObj.id] - Id of the button
 * @param {string} [buttonObj.group] - Group of the button
 * @param {string} [buttonObj.title] - Title of the button
 * @param {string} [buttonObj.img] - Image of the button
 * @param {string} [buttonObj.icon] - Icon of the button
 * @param {string} [buttonObj.elm] - HTML element of the button
 * @param {boolean} [buttonObj.checked] - Checked state of the button
 * @param {Function} [buttonObj.onClick] - Click event handler of the button
 * @param {Function} [buttonObj.onExpanderClick] - Click event handler of the expander button
 * @returns {jQuery} - The button element
 */
ui.Toolbar.prototype.addButtonWithMenu = function(buttonObj, menu) {
	buttonObj = buttonObj || {};
	menu = common.nwUtil.createMenu(menu);
	buttonObj.id 	= buttonObj.id || common.generateId();
	buttonObj.group = buttonObj.group || this.defaultGroup;
	buttonObj.title = buttonObj.title || "";
	buttonObj.checked = buttonObj.checked || false;
	...
	if (buttonObj.html) {
		...
		buttonObj.elm	= buttonObj.elm || `<span class="menu-button buttonGroup"><button class="hasMenuSide mainButton" data-tranattr="title" title="${buttonObj.title}" >${buttonObj.text}</button><button class="menuSide"></button></span>`
		buttonObj.elm 	= $(buttonObj.elm);
	}
	buttonObj.checked = buttonObj.checked || false;
	buttonObj.onClick = buttonObj.onClick || function(){};
 ...
 * When the button is clicked will show a menu
 * @param {Object} buttonObj - Button object
 * @param {Object} menu - Menu object
 * @param {string} [buttonObj.id] - Id of the button
 * @param {string} [buttonObj.group] - Group of the button
...
ui.Toolbar.prototype.addMenuButton = function(buttonObj, menu) {
	...
	if (buttonObj.html) {
		...
		buttonObj.elm	= buttonObj.elm || `<button class="menu-button buttonExpand" data-tranattr="title" title="${buttonObj.title}">${buttonObj.text}</button>`
		buttonObj.elm 	= $(buttonObj.elm);
	}
	buttonObj.checked = buttonObj.checked || false;
	buttonObj.onClick = buttonObj.onClick || function(){};
	...
	buttonObj.elm.on("click", function(ev) {
		ev.preventDefault();
		const thisMenu = common.nwUtil.createMenu(menu);
        var $wrapper = buttonObj.elm;
        var left = parseInt($wrapper.offset().left);
        var top = parseInt($wrapper.offset().top + $wrapper.outerHeight());
        if (thisMenu) thisMenu.popup(left, top);
		buttonObj.onClick.apply(this, arguments);
	});
	if (this.buttons[buttonObj.id]) buttonObj.elm.remove();
...
ui.Toolbar.prototype.show = function(triggerEvent){
	if (typeof triggerEvent == 'undefined') triggerEvent = true;
	ui.onDOMReady(()=> {
		if (triggerEvent) this.elm.trigger("beforeShow");
		this.container.find(".toolbar").addClass("hidden")
		this.elm.removeClass("hidden")
		if (triggerEvent) this.elm.trigger("show");
	})
}
 ...
 */
class RibbonMenu extends BasicEventHandler{
	constructor() {
		super();
		var that = this;
		/**
 ...
 */
RibbonMenu.prototype.init = function(){
	this.onReady(()=> {
		...
		this.add("translator", {
			title : "Translator",
			icon: "icon-globe-speech",
			permanent : true,
			toolbar : new ui.Toolbar("translator")
		});

		// hide all .toolbarGroup>.toolbar except home
		$(".toolbarGroup>.toolbar").not("[data-id='home']").addClass("hidden");
...
RibbonMenu.prototype.getRibbon = function(id) {
	return this.ribbons[id];
}

/**
 * Selects a ribbon and triggers its onClick event.
 * @param {jQuery|string} $ribbon - The ribbon element or its ID.
 * @param {boolean} [trigger=true] - Whether to trigger the onClick event.
 */
RibbonMenu.prototype.select = function($ribbon, trigger) {
	console.warn("RibbonMenu.select", $ribbon, trigger);
	if (typeof trigger == 'undefined') trigger = true;
	if (typeof $ribbon == 'string') {
		$ribbon = $("#applicationBar .ribbonMenu[data-id='"+$ribbon+"']");
	}
	const thisId = $ribbon.data("id");
	const thisRibbon = this.getRibbon(thisId);

	// skip if already selected
	if ($ribbon.hasClass("active")) return;

	/**
	 * Event triggered before a ribbon is selected.
	 * @event RibbonMenu#beforeSelect
	 */
	if (trigger) $ribbon.trigger("beforeActive");

	const $ribbons = $("#applicationBar .ribbonMenu");
     ...
     * @event RibbonMenu#select
     * @type {string}
     * @property {string} id - The ID of the selected ribbon.
     */
	if (trigger) this.trigger("select", thisId);
	/**
	 * Event triggered when a ribbon is active.
	 * @event RibbonMenu#active
	 */
	if (trigger) $ribbon.trigger("active");


	if (thisRibbon?.toolbar) {
		thisRibbon.toolbar.show(trigger);
	}
	if (trigger == false) return;

	if (typeof thisRibbon.onClick !== 'function') return;
	for (let idx in this.ribbons) {
		let ribbon = this.ribbons[idx];
		if (typeof ribbon.onBlur !== 'function') continue;
		if (ribbon.id == thisId) continue;
		ribbon.onBlur.call(this);
	}
 ...
 */
RibbonMenu.prototype.onReady = function(fn) {
    // see if DOM is already available
    if (document.readyState === "complete" || document.readyState === "interactive") {
        // call on next available tick
        setTimeout(fn, 1);
 ...
 * @param {string} id - The ID of the ribbon.
 * @returns {boolean} True if the ribbon exists, false otherwise.
 */
RibbonMenu.prototype.isExist = function(id) {
 ...
 * @param {string} id - The ID of the ribbon.
 * @param {Object} options - The options for the ribbon.
 * @param {string} [options.title] - The title of the ribbon.
 * @param {string} [options.icon] - The icon class for the ribbon.
 * @param {boolean} [options.permanent=false] - Whether the ribbon is permanent.
 * @param {Function} [options.onClick] - The onClick event handler.
 * @param {Function} [options.onBlur] - The onBlur event handler.
 * @param {ui.Toolbar} [options.toolbar] - The toolbar associated with the ribbon.
 * @returns {Object} The added ribbon configuration.
 */
RibbonMenu.prototype.add = function(id, options) {
	if (typeof id !== "string") return;
	if (id == "") return;
	options 			= options || {};
	options.onClick 	= options.onClick || function(){};
	options.title 		= options.title || options.id;
	options.permanent	= options.permanent || false; // cannot be removed
	...
	this.onReady(()=>{
		console.log("%c Adding ribbon menu : "+id, "color:aqua");
		...
		$template.empty();
		if (options.icon !== "") $template.prepend('<i class="'+options.icon+'"></i>');
		$template.append($(`<span>`+options.title+`</span>`));
 ...
 * @param {string} id - The ID of the ribbon to remove.
 */
RibbonMenu.prototype.remove = function(id) {
	if (!this.ribbons[id]) return;
	if (this.ribbons[id].permanent) return;
 ...
 */
RibbonMenu.prototype.clear = function() {
	// remove all
	for (var key in this.ribbons) {
		if (this.ribbons[key].permanent) continue;
		this.remove(key);
	}

	// set ribbon back to home
	this.select("start", false);

}

ui.ribbonMenu = new RibbonMenu();
...
NewProjectDialog.prototype.addMenu = function(options) {
	...
	var $mainSlide = this.$elm.find("[data-slideId=1] .projectTypeList");
	...
	var $slide1 = $('<li class="dialogSelection" data-gotoSlide="'+options.goToSlide+'">\
...
NewProjectDialog.prototype.reset = function() {
	...
	// init filter
	const $searchProjectType = this.$elm.find(".searchProjectType");
	const that = this;
	$searchProjectType.off("focus").on("focus", function() {
		console.log("initializing search menu");
		ui.showBusyOverlay();
		that.searchList = new window.List(that.$elm[0], {
			listClass: 'projectTypeList',
			valueNames: [
				'descriptionBar',
				'contains',
				{ data: ['keyword'] }
		...
		that.searchList.reIndex();
		that.searchList.on("searchStart", function() {
...
ui.openGlobalAutomationContext = function($elm) {
	$elm ||= $("#globalAutomationContext");
	const menu = [];
	const config = sys.getConfig("codeEditor/global")?.quickLaunch || [];
	for (let i=0; i<config.length; i++) {
		let filePath = config[i];
		let filename = common.getFilename(filePath);
		menu.push({
			label: filename,
			click: () => {
				var conf = confirm(t("Are you sure want to execute the following script?")+"\n"+filename);
				if (!conf) return;
				trans.runCustomScript("global", filePath);
			}
		});
	}
	...
	if (menu.length < 1) {
		menu.push({
			label: t("No script found"),
			disabled: true
		});
	}
	menu.push({type:"separator"});
	menu.push({
		label: t("Open Script Editor"),
		click: () => {
			var options = {
				workspace: "global"
			}
	...
	menu.push({
		label:  t("How to add Automation here"),
		click: () => {
			nw.Shell.openExternal("https://dreamsavior.net/docs/translator/execute-script/pin-your-automation-to-quickly-launch-from-translator/");
		}
	});
	menu.push({type:"separator"});
	menu.push({
		label: t("Clear Quick Launch"),
		click: () => {
			var conf = confirm(t("Are you sure want to clear all quick launch scripts?"));
			if (!conf) return;
			sys.setConfig("codeEditor/global", {quickLaunch:[]});
			sys.saveConfig();
		}
	});

	return this.popupMenu(menu, $elm);
}
...
// ==============================================================
	...
	*/
	...
	$(".button-execute-script-side").on("click", function() {
		ui.openGlobalAutomationContext($(this).parent());
	});
	...
	$(document).on("windowResizeStop", function() {
		// var maxHeight = $(window).height()-120;
		// $("#cellSelectionInfo").resizable( "option", "maxHeight", maxHeight );
	...
	$(document).on('keydown', function(e) {
		...
		if (e.ctrlKey && e.shiftKey ) {
			switch(keyCode) {
				...
				case 68 : //shift d
					e.preventDefault();
					console.log("Pressing CTRL+shift+d");
