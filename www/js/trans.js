...
Trans.prototype.getSl = function() {
	...
	var sl 					= this.getOption("sl") || sys.config.default?.sl || $DV.config.sl;
	return sl;
}
Trans.prototype.getTl = function() {
	...
	// this.project.options 	= this.project.options || {}
	sys.config.default 		= sys.config.default || {};
	var tl 					= this.getOption("tl") || sys.config.default?.tl || $DV.config.tl;
	return tl;
}

/**
 * Set the source language of the current project
 * @param {String} value - The source language's code
 * @since 7.3.26
 */
Trans.prototype.setSl = function(value) {
	if (!this.project) return;
	this.setOption("sl", value);
}

/**
 * Set the target language of the current project
 * @param {String} value - The target language's code
 * @since 7.3.26
 */
Trans.prototype.setTl = function(value) {
	if (!this.project) return;
	this.setOption("tl", value);
}
...
Trans.prototype.loadJSON = async function(filePath, options) {
	...
	var jsonData;
	try {
		let loadedFile = await common.fileGetContents(filePath, "utf8", false);
 ...
 * @param {Function} options.onDone - Triggered when done 
 */
Trans.prototype.importTpp = async function(file, options) {
	if (typeof file=="undefined") return false;
	
	options = options||{};
	options.onDone = options.onDone||function() {};

	// ensure staging path is exist
	const staggingPath = sys.getConfig("stagingPath");
	if (!await common.isDir(staggingPath)) {
		await common.mkDir(staggingPath);
	}
 ...
 */
 Trans.prototype.export = async function(file, options) {
	...
	if (["csv", "xlsx", "xls", "ods", "html", "RPGMakerTrans"].includes(options.mode) || thisEngine=="spreadsheet") {
		this.legacyExport(file, options);
	}
	console.log("Export process is finished");
}

Trans.prototype.legacyExport = async function(file, options) {

	// below this point is for legacy mode
	const autosavePath = await trans.autoSave();
	ui.log.show();
	await ui.log("Exporting project to "+options.mode+" format with legacy mode");
	trans.project.options = trans.project.options || {}
	trans.project.options.init = trans.project.options.init || {};
	const projectOption = Object.assign(trans.project.options.init, options.options);
...
Trans.prototype.revertToOriginal = async function(destinationPath, options={}) {
	...
	ui.log.addButtons("Open folder", function() {
 ...
 */
 Trans.prototype.importSheet = async function(paths, columns, options) {
	...
	options.indexes = this.buildIndexes(options.files, false, {stripCarriageReturn:true});
...
/**
 * Select file by given function
 * @param {Function} filter - Filter function
 */
Trans.prototype.selectObjectsByFilter = function(filter) {
	const $checkBoxes = $("#fileList .fileCheckbox");
	if (typeof filter !== "function") return;
	$checkBoxes.each(function() {
		const $this = $(this);
		if (filter($this.closest("li").data("id"), $this)) $this.prop("checked", true).trigger("change");
	});
}
...
Trans.prototype.updateRunScriptMenu = function() {
	...
	if (Object.keys(forEachRowRunItems).length > 0) {
		forEachRowRunItems["sep0"] = "---------";
		forEachRowRunItems["clearQuickLaunch"] = {
			name: t("Clear quick launch"),
			icon: 'context-menu-icon icon-trash',
			callback: (key, opt) => {
				var conf = confirm(t("Are you sure want to clear quick launch?"));
				if (!conf) return;
				sys.setConfig("codeEditor/rowIterator", {quickLaunch:[]});
				sys.saveConfig();
	...
	if (Object.keys(forEachObjectRunItems).length > 0) {
		forEachObjectRunItems["sep0"] = "---------";
		forEachObjectRunItems["clearQuickLaunch"] = {
			name: t("Clear quick launch"),
			icon: 'context-menu-icon icon-trash',
			callback: (key, opt) => {
				var conf = confirm(t("Are you sure want to clear quick launch?"));
		...
		forEachObjectRunItems["howToAdd"] = {
			name: t("How to add Automation here"),
			icon: 'context-menu-icon icon-help',
			callback: (key, opt) => {
				nw.Shell.openExternal("https://dreamsavior.net/docs/translator/execute-script/pin-your-automation-to-quickly-launch-from-translator/");
	...
	this.fileSelectorMenu = {
		...
		"withSelected": {
			...
			items: {
				...
				"sep0-1":"---------",
				"wrapText": {"name" : t("Wrap texts"), "icon": "context-menu-icon icon-wordwrap"},
				"trim": {"name" : t("Trim"), "icon": "context-menu-icon icon-article"},
				"padding": {"name" : t("Auto padding"), "icon": "context-menu-icon icon-list-nested"},
				"createScript": {
			...
			var thisCallback = function(key, options) {
				switch (key) {
					...
					case "selectProgressGT" :
						const progress = prompt(t("Select progress greater than"), "0");
						...
						trans.selectObjectsByFilter((id, $this) => {
							const thisObj = trans.getObjectById(id);
							if (!thisObj.progress) return false;
							const thisProgress = thisObj.progress.translated / thisObj.progress.length * 100;
							return thisProgress >= parseInt(progress);
						});
...
/**
 * Add a new array of data into a file
 * @param  {Any} file - File ID or File Object or undefined
 * @param  {String[]} data - Array of data to be inserted
 * @returns {Integer} the index of the new inserted data
 */
Trans.prototype.appendRow = function(file, column=[]) {
	var thisObj
	if (typeof file == "object") {
		thisObj = file;
	} else {
	...
	thisObj.indexIds ||= {};
	if (Array.isArray(column) == false) return -1;
	if (column.length < 1) return -1;

	const keyString = column[this.keyColumn];
	if (typeof keyString !== "string") return -1;
	if (!keyString) return -1;
	...
	// ensure the column length is the same
	// trim column to the same length of trans.colHeaders.length
	column.length = trans.colHeaders.length;
	const newRow = common.clone(column);
...
Trans.prototype.appendTextToReference = function(text) {
	if (!this.isInProject()) return false;
	if (typeof text !== 'string') return false;
	if (!text.trim()) return trans.alert("Cannot add empty text to reference. Please try again with a text selected, or use <b>CTRL+SHIFT+D</b> to add the selected row into reference.");
...
Trans.prototype.appendSelectedRowToReference = function() {
	const selectedRow = common.gridSelectedRows();
	if (!this.isInProject()) return;
	if (!selectedRow?.length) return this.alert("No row selected.");
	if (!this.getObjectById("Common Reference")) return this.alert("This project don't have <b>Common Reference</b> file. Please create one first.");
	
	let insertedRow = 0;
	for (let i=0; i<selectedRow.length; i++) {
		if (this.appendRow("Common Reference", this.getData()[i]) > -1) {
			insertedRow++;
		};
	}

	this.alert(insertedRow + " row(s) added to Common Reference.");
	return true;
}
...
Trans.prototype.getProgressColumnIndices = function() {
	const setting = sys.getConfig("progressColumnIndices");
	if (!setting) return;
	let rowList = setting.replaceAll(" ", "").split(",");

	// ensure rowList is an array of number
	rowList = rowList.map(function(row) {
		return parseInt(row);
	});

	// make the array unique
	return common.arrayUnique(rowList);
...
Trans.prototype.countTranslated = function(file) {
	...
	const progressColumnIndices = this.getProgressColumnIndices();
	console.log("Progress column indices", progressColumnIndices);
	...
	for (var i=0; i<file.length; i++) {
		...
		for (var row=0; row<thisData.length; row++) {
			...
			if (this.lineByLineMode) {
				// DO NOT USE, evaluating by lines instead of rows
				try {
				...
				if (this.rowHasTranslation(thisData[row], this.keyColumn, progressColumnIndices)) result[file[i]].translated++;
			}
...
Trans.prototype.buildIndexes = function(files, lineByLine=false, options={}) {
	...
	for (let i=0; i<files.length; i++) {
		...
		for (let key in thisIndexIds) {
			...
			result[key].push(thisRef);
			
			if (key.includes("\r") && options.stripCarriageReturn) {
				// remove carriage return from the key
				const strippedKey = key.replaceAll("\r", "");
				result[strippedKey] = result[strippedKey] || [];
				result[strippedKey].push(thisRef);
			}
		}
	}
 ...
 */
 Trans.prototype.getTranslationData = function(transData, options) {
	...
	*/
	...
	options.collectUntranslated ||= false;
	...
	for (let fileId in transData.project.files) {
		...
		for (var row=0; row<thisFiles.data.length; row++) {
			...
			try {
				...
				if (!options.collectUntranslated) {
					...
					if (options.objectGrouping) {
						...
						let thisKey = thisFiles[options.groupIndex] ? thisFiles[options.groupIndex]+contextSeparator+originalWord : originalWord
					...
					if (options.objectGrouping) {
						if (thisFiles[options.groupIndex]) {
							...
							//console.log("assigning to child object",thisFiles[options.groupIndex]);
							transGroup[thisFiles[options.groupBy]].translationPair[thisFiles[options.groupIndex]] = transGroup[thisFiles[options.groupBy]].translationPair[thisFiles[options.groupIndex]] || {};
							transGroup[thisFiles[options.groupBy]].translationPair[thisFiles[options.groupIndex]][originalWord] = trans.wordWrapText(thisTranslation, thisTag, options.wordWrapByTags) || "";
						} else {
							//direct translation string
							transGroup[thisFiles[options.groupBy]].translationPair[originalWord] = trans.wordWrapText(thisTranslation, thisTag, options.wordWrapByTags) || "";
						}
		
					} else {
						let thisKey = thisFiles[options.groupIndex] ? thisFiles[options.groupIndex]+contextSeparator+originalWord : originalWord
						transGroup[thisFiles[options.groupBy]].translationPair[thisKey] = trans.wordWrapText(thisTranslation, thisTag, options.wordWrapByTags) || "";
					}
				}
 ...
 * @returns {Trans}
 */
 Trans.prototype.shouldPreserveDuplicateRows = function(trans) {
	trans = trans || this;
	var project = trans.project || trans;
	if (typeof project !== 'object' || project === null) return false;
	var options = project.options || {};
	if (options.preserveDuplicateRows === true) return true;
	if (typeof options.srpgstudio === 'object' && options.srpgstudio !== null) {
		if (options.srpgstudio.preserveDuplicateRows === true) return true;
	}
	return false;
}
 ...
 Trans.prototype.sanitize = function(trans) {
	...
	if (this.shouldPreserveDuplicateRows(trans)) {
		trans.project.isDuplicatesRemoved = false;
		return trans;
	}
 ...
 */
 Trans.prototype.translateFromArray = function(obj, columns, options) {
	...
	console.log("options.indexes", options.indexes);
 ...
 * @param {String[]} cells - Single dimensional array representing rows
 * @param {Number} [keyColumn=0] 
 * @param {Number|Number[]|undefined} [indexToCheck] - Index or indices to check for translation
 * @returns {Boolean} True if has translation
 */
Trans.prototype.rowHasTranslation = function(cells=[], keyColumn= this.keyColumn, indexToCheck) {
	if (!cells || !cells.length) return false;
	keyColumn = this.keyColumn || 0;

	if (typeof indexToCheck == "undefined" || indexToCheck === null) {
		for (let i=0; i<cells.length; i++) {
			if (i == keyColumn) continue;
			if (cells[i]) return true;
		}
	} else {
		if (!Array.isArray(indexToCheck)) indexToCheck = [indexToCheck];
		for (let i=0; i<indexToCheck.length; i++) {
			if (indexToCheck[i] == keyColumn) continue;
			if (cells[indexToCheck[i]]) return true;
		}
	}
 ...
 * Executex the translator engine's translateAll function if available
 ...
 * @param {Boolean} [options.overwrite=false] - Whether to overwrite or not if the destination cell is not empty
 * @param {Boolean} [options.ignoreTranslated=false] - Whether to skip processing or not if the row already has translation
 * @param {Object} [options.translatorOptions] - Translator specific options
 */
 Trans.prototype.translateAll = async function(translator, options) {
	const activeTranslator = this.getTranslatorEngine(translator);
	this.buildIndex("Common Reference", true);
	if (activeTranslator.translateAll == "function") {
		return activeTranslator.translateAll(options);
	}
	
	if (activeTranslator.translatorType == "v2") {
		return this.batchTranslate(activeTranslator, options);
	}
...
Trans.prototype.batchTranslate = async function(activeTranslator, options) {
		// use V2 translator mechanism
		const batchTranslate = new BatchTranslate(activeTranslator, options);
		...
		await batchTranslate.translateAll();
		await ui.log.end();
		await this.onBatchTranslationDone(options);
		return;
 ...
 Trans.prototype.translateAllByRows = async function(translator, options) {
	...
	var processPart = async function() {
		...
		if (thisTranslator.job.batch.length < 1) {
			...
			ui.loadingEnd();
			await trans.onBatchTranslationDone(options);
 ...
 */
 Trans.prototype.onBatchTranslationDone = async function(options) {
	...
	if (typeof engines !== "undefined") {
		await engines.handler('onBatchTranslationDone').call(this, options);
	}
...
Trans.prototype.getTranslatorEngine = function(id) {
	...
	if (id instanceof TranslatorEngine) return id;
...
/**
 * Set active translator engine
 * @param {String|TranslatorEngine} translator - Translator engine ID or object
 * @since 7.3.26
 */
Trans.prototype.setActiveTranslator = function(translator) {
	if (typeof translator == 'string') {
		translator = this.getTranslatorEngine(translator);
	}
	if (!translator?.id) return;
	this.setOption("translator", translator.id);
}
...
/**
 * Set parameters by row id & file id
 * @param {Number} row - The row id
 * @param {Object} parameters - The parameters to be set
 * @param {String} [file=this.getSelectedId()] - The file id
 */
Trans.prototype.setParametersByRow = function(row, parameters, file) {
	file = file || this.getSelectedId();
	const thisObj = this.getObjectById(file);
	thisObj.parameters = thisObj.parameters || [];
	thisObj.parameters[row] = parameters;
}
 ...
 * @param {Boolean} [untranslated=false] - Whether to display untranslated text or not
 * @returns {String} row info
 */
Trans.prototype.getRowInfoText = function(row, full=false, file="", untranslated=false) {
	file = file || this.getSelectedId();
	...
	if (full) {
		...
		for (let i in thisParam) {
			...
			if (untranslated) {
				if (thisParam[i].rowInfoText) result.push(thisParam[i].rowInfoText);
			} else {
	...
	for (let i in thisParam) {
		...
		if (untranslated) {
			if (thisParam[i].rowInfoText) result = thisParam[i].rowInfoText;
		} else {
...
/**
 * Set row info text (eg. Character name) to the object's parameters
 * @param {Number} row - Row id
 * @param {String} text - Text to be set
 * @param {String} [file=this.getSelectedId()] - File id
 */
Trans.prototype.setRowInfoText = function(row, text, file) {
	file = file || this.getSelectedId();
	const thisParam = this.getParamatersByRow(row, file);
	if (!thisParam) return false;
	thisParam[0] ||= {};
	thisParam[0].rowInfoText = text;
	return true;
}
